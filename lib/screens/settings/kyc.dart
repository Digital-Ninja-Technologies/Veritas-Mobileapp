  ...
  Widget _buildStep() {
    switch (_step) {
      case 0:
        return _DocTypeStep(
            docType: _docType,
            onSelect: (d) => setState(() => _docType = d),
      ...
      case 1:
        return _ScanStep(
            ...
            sub:
                'Place the front of your ${_docType ?? 'document'} in the frame',
            onNext: () => setState(() => _step = 2));
      case 2:
        return _ScanStep(
            title: 'Scan back of document',
            icon: Icons.flip,
            sub: 'Flip your document and scan the back',
      ...
      case 3:
        return _ScanStep(
            ...
            sub:
                'Look straight at the camera and make sure your face is clearly visible',
            onNext: () => setState(() => _step = 4),
            selfie: true);
      case 4:
        return _SubmittedView(onDone: () => Navigator.of(context).pop());
      default:
...
class _DocTypeStep extends StatelessWidget {
  ...
  const _DocTypeStep(
      {required this.docType, required this.onSelect, required this.onNext});

  @override
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('Which document will you use?',
          style: TextStyle(
              fontSize: 20,
              fontWeight: FontWeight.w800,
              color: AppColors.darkText,
      ...
      const Text(
          'Make sure the document is valid, not expired, and issued by a government.',
          style: TextStyle(
              fontSize: 13.5, color: AppColors.subText2, height: 1.5)),
            ...
            child: GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: Colors.white,
                  border: Border.all(
                      color: docType == d ? AppColors.dark : AppColors.border,
                      width: 1.5),
...
class _ScanStepState extends State<_ScanStep> {
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
                color: _captured ? AppColors.greenDark : AppColors.border,
                width: 2,
                style: _captured ? BorderStyle.solid : BorderStyle.solid),
      ...
      const SizedBox(height: 28),
      VButton(
          label: _captured ? 'Looks good, continue' : 'Capture',
          onTap: _captured
              ? widget.onNext
  ...
  Widget build(BuildContext context) {
    return Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      const Text(
        ...
        style:
            TextStyle(fontSize: 14.5, color: AppColors.subText2, height: 1.6),
      ),
      const SizedBox(height: 28),
      Container(
        padding: const EdgeInsets.all(16),
